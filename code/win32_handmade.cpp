                      ...
                      win32_sound_output *SoundOutput, real32 TargetSecondsPerFrame)
{
    ...
    {
        Assert(Markers[MarkerIndex].PlayCursor < SoundOutput->SecondaryBufferSize);
        int PlayCursorX = PadX + (int)(C * (real32)Markers[MarkerIndex].PlayCursor);
        int WriteCursorX = PadX + (int)(C * (real32)Markers[MarkerIndex].WriteCursor);
...
WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CommandLine, int ShowCode)
{
    ...
    // TODO(george): Let's think about running non-frame-quantized for audio latency...
    // TODO(george): Let's use the write cursor delta from the play cursor to adjust 
            ...
            if (Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage)
            {
                ...
                {   
                    ...
                    *NewKeyboardController = {};
                    ...
                    // NOTE(george): Compute how much sound to write and where
                    DWORD ByteToLock = 0;
