...
EndDebugStatistic(debug_statistic *Stat)
{
    ...
    {
        Stat->Min = Stat->Max = Stat->Avg = 0.0;
    }
}
                  ...
                  debug_element *RootElement)
{
        ...
        Index++)
    {
        ...
        debug_statistic *Stat = &Entry->Stats;
        debug_element *Element = Entry->Element;
                ...
                debug_element *RootElement)
{
    ...
    if(FrameCount > 0)
    {
        PushRect(&DebugState->RenderGroup, DebugState->BackingTransform, TotalRect, 0.0f, V4(0.0f, 0.0f, 0.0f, 0.25f));
            ...
            FrameIndex++)
        {
            ...
            PushRectOutline(&DebugState->RenderGroup, DebugState->UITransform, RegionRect, 0.0f, V4(0.5f, 0.5f, 0.5f, 1.0f), 2.0f);
                 ...
                 u32 FrameOrdinal)
{
    ...
    switch(Element->Type)
    {
        ...
        case DebugType_FrameSlider:
        {
            ...
            ActionButton(Layout, "Oldset", 
                         SetUInt32Interaction(DebugID, &DebugState->ViewingFrameOrdinal, DebugState->OldestFrameOrdinal));
            ActionButton(Layout, "Most Recent", 
                         SetUInt32Interaction(DebugID, &DebugState->ViewingFrameOrdinal, DebugState->MostRecentFrameOrdinal));
            EndRow(Layout);
...
DrawTrees(debug_state *DebugState, v2 MouseP)
{
        ...
        MenuItemIndex++)
    {
        ...
        rectangle2 TextBounds = GetTextSize(DebugState, Text);
        TextOutAt(DebugState, TextP - 0.5f*GetDim(TextBounds), Text, ItemColor);
    }
...
internal debug_element *GetElementFromEvent(debug_state *DebugState, debug_event *Event, debug_variable_link *Parent, u32 Op);
internal void
DEBUGMarkEditedEvent(debug_state *DebugState, debug_event *Event)
{
    if(Event)
    {
        ...
        GlobalDebugTable->EditEvent.GUID = GetElementFromEvent(DebugState, Event, 0, DebugElement_AddToGroup|DebugElement_CreateHierarchy)->OriginalGUID;
    }
}
...
CreateVariableLink(debug_state *DebugState, u32 NameLength, char *Name)
{
    ...
    Link->Name = NameLength ? PushAndNullTerminateString(&DebugState->DebugArena, NameLength, Name) : 0;
    Link->Element = 0;
        ...
        else
        {
            ...
            switch(Event->Type)
            {
                case DebugType_BeginBlock:
                {
                    ...
                    debug_element *Element = GetElementFromEvent(DebugState, Event, DebugState->ProfileGroup, DebugElement_AddToGroup);
                ...
                case DebugType_OpenDataBlock:
                {
                    ...
                    debug_parsed_name ParsedName = DebugParseName(Event->GUID);
                    debug_variable_link *Group = GetGroupForHierarchicalName(DebugState, DefaultParentGroup, ParsedName.Name, true);
           ...
           game_assets *Assets, u32 MainGenerationID, uint32 Width, uint32 Height)
{
    ...
    if(DebugState)
    {
        if(!DebugState->Initialized)
        {
            ...
            // recycling works.
            SubArena(&DebugState->PerFrameArena, &DebugState->DebugArena, 1*1024*512);
#endif
