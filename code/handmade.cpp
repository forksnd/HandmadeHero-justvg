...
{
    world_difference Diff = Substract(GameState->World, &EntityLow->Sim.P, &GameState->CameraP);
    v2 Result = Diff.dXY;
...
AddPlayer(game_state *GameState)
{
    ...
    Entity.Low->Sim.SwordLowIndex.Index = Sword.LowIndex;
...
UpdateSword(game_state *GameState, entity Entity, real32 dt)
{
    ...
    if(Entity.Low->Sim.DistanceRemaining < 0)
    {
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    for (int ControllerIndex = 0; ControllerIndex < ArrayCount(Input->Controllers); ControllerIndex++)
    {
        ...
        else
        {
            ...
            if((dSword.X != 0) || (dSword.Y != 0))
            {
                ...
                {
                    world_position SwordP = ControllingEntity.Low->Sim.P;
                    ChangeEntityLocation(&GameState->WorldArena, GameState->World, ControllingEntity.Low->Sim.SwordLowIndex, LowSword, 0, &SwordP);
    ...
    for(uint32 EntityIndex = 0; EntityIndex < SimRegion->EntityCount; EntityIndex++)
    {
        ...
        low_entity *LowEntity = GameState->LowEntities + Entity->StorageIndex;
