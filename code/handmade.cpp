...
InitializePlayer(game_state *GameState, uint32 EntityIndex)
{
    ...
    Entity.Dormant->P.Offset_ = {0.0f, 0.0f};
...
MovePlayer(game_state *GameState, entity Entity, real32 dt, v2 ddP) 
{
    ...
    if(ddPLength > 1.0f)
    {
        ddPLength *= 1.0f / SquareRoot(ddPLength);
    }
        ...
        for(uint32 AbsTileY = MinTileY; AbsTileY <= MaxTileY; AbsTileY++)
        {
            for(uint32 AbsTileX = MinTileX; AbsTileX <= MaxTileX; AbsTileX++)
            {
                ...
                if(!IsTileValueEmpty(TileValue))
                {
                    ...
                    tile_map_difference RelOldPlayerP = Substract(TileMap, &Entity->P, &TestTileP);
    ...
    //  
    // NOTE(george): Update camera/player Z based on last movement. 
    // 
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    {
        // TODO(george): Fix this to work in the camera space!
#if 0
    ...
    for(uint32 EntityIndex = 0; EntityIndex < GameState->EntityCount; EntityIndex++)
    {
        ...
        if(GameState->EntityResidence[EntityIndex] == EntityResidence_High)
        {
            ...
            real32 PlayerG = 1.0f;
            real32 PlayerB = 0.0f;
            real32 PlayerGroundPointX = ScreenCenterX + HighEntity->P.X;
            real32 PlayerGroundPointY = ScreenCenterY - HighEntity->P.Y;
            ...
            DrawBitmap(Buffer, &HeroBitmaps->Hero, PlayerGroundPointX, PlayerGroundPointY, HeroBitmaps->AlignX, HeroBitmaps->AlignY);                  
        }
    }
}
